...
const data = await fetch("https://raw.githubusercontent.com/naari3/nomifactory-ja/main/src/assets/questbook/lang/en_us.lang");
const text = await data.text();
const valuableLines = text.split("\n").filter(line => line !== "").filter(line => !line.startsWith("#"));
...
const translatedResult = (isTranslatedResultFileExists ? JSON.parse(await fs.readFile(translatedResultFileName, "utf-8")) : {}) as { [key: string]: { translatedCount: number, allCount: number } };

const currentDate = new Date().toLocaleString("ja-JP", { timeZone: 'JST' });
translatedResult[currentDate] = result;

await fs.writeFile(translatedResultFileName, JSON.stringify(translatedResult, null, 2));
